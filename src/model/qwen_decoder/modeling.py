...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    # query.shape -> [B, heads, seq, hidden] # rel_score -> [B, seq]
    # TODO: add relevant scores
    if kwargs.get("relevant_scores", None) is not None: 
        relevant_scores = kwargs["relevant_scores"].unsqueeze(1).unsqueeze(-1).to(query.dtype) # [B, seq] - > [B, 1, seq, 1]
...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: IConfig, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        #self.rope_modulator = RoPEImportanceModulator(init_alpha=0.5, learnable=False)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        #breakpoint() # [1, 1700, 12, 128]
        attn_output = attn_output.reshape(*input_shape, -1).contiguous()
        #breakpoint() # [1, 1700, 1536]
...
class Qwen2RotaryEmbedding(nn.Module):
    def __init__(self, config: IConfig, device=None):
        super().__init__()
...
class IModel(Qwen2PreTrainedModel):
    """
...
class IModelForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    _tp_plan = {"lm_head": "colwise_rep"}

    def __init__(self, config):
        super().__init__(config)
        self.model = IModel(config)
        self.vocab_size = config.vocab_size
...
class Qwen2ForQuestionAnswering(Qwen2PreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.transformer = IModel(config)
